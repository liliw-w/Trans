...
# slice(gene_repurpose_expand, which(gene_repurpose_expand$target == "ERBB3")) %>% View()
...
target_gene_trait_eqtlgen <- apply(target_gene_trait, 1, function(x){
  ...
  filter(eqtlgen_snp, SNPChr == tmp_chr) %>%
    ...
    arrange(s) %>%
    slice(1) %>%
    select(SNPChr, SNPPos, s, e, target)
}) %>%
  bind_rows()
...
target_gene_trait_trans <- apply(target_gene_trait, 1, function(x){
  ...
  tibble(
    "n_trans_module" = pull(tmp_res, gene_module) %>% unique() %>% length(),
    "trans_module" = pull(tmp_res, gene_module) %>% unique() %>% paste(collapse = ";"),
    "n_trans_module_immune" = filter(tmp_res, if_immune_blood_anno) %>% distinct(gene_module) %>% pull(gene_module) %>% length(),
    "trans_module_immune" = filter(tmp_res, if_immune_blood_anno) %>% distinct(gene_module) %>% pull(gene_module) %>% paste(collapse = ";")
  )
}) %>%
  bind_rows() %>%
...
data.table::fwrite(
  ...
  file = "target_gwas_eqtlgen_trans_module_coexp.txt",
  sep = "\t", quote = FALSE
)
...
target_gene_trait_trans2 <- apply(target_gene_trait, 1, function(x){
  ...
  tibble(
    "n_trans_module" = pull(tmp_res, module_msig) %>% unique() %>% length(),
    "trans_module" = pull(tmp_res, module_msig) %>% unique() %>% paste(collapse = ";"),
    "n_trans_module_immune" = filter(tmp_res, if_immune_blood_anno) %>% distinct(module_msig) %>% pull(module_msig) %>% length(),
    "trans_module_immune" = filter(tmp_res, if_immune_blood_anno) %>% distinct(module_msig) %>% pull(module_msig) %>% paste(collapse = ";")
  )
}) %>%
  bind_rows() %>%
  cbind(target_gene_trait, .) %>%
...
data.table::fwrite(
  ...
  file = "target_gwas_eqtlgen_trans_module_hallmark.txt",
  sep = "\t", quote = FALSE
)
