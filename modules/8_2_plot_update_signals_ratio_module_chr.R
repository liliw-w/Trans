###############################################################
########### plot updated unique significant SNPs ###########
###############################################################
# load packages -----
rm(list = ls())
library(data.table)
...
# I/O & paras -----
ratio_thre <- 50
file_signal <- paste0('postanalysis/signal_rm_infl_ratio_', ratio_thre, '_module_chr.txt')

## output -----
file_plot_module <- paste0('plot/signal_module_rm_infl_ratio_', ratio_thre, '_module_chr.pdf')
file_plot_chr <- paste0('plot/signal_chr_rm_infl_ratio_', ratio_thre, '_module_chr.pdf')
file_plot_module_chr <- paste0('plot/signal_module_chr_rm_infl_ratio_', ratio_thre, '_module_chr.pdf')
...
signal %>% count(module, SNPChr) %>% arrange(desc(n))
...
signal$module <- factor(signal$module)
...
ggsave(file_plot_module, width = 5, height = 4)


# 2. plt how many signals are on each chr? -----
signal$SNPChr <- factor(signal$SNPChr)
...
ggsave(file_plot_chr, width = 6, height = 4)
...
ggsave(file_plot_module_chr, width = 5, height = 5)
