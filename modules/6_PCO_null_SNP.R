...
file_cross_map_genes <- "/project2/xuanyao/llw/eQTLGen/DGN_est_Sigma/cross_map_genes.rds"
file_num_gene_snp_used_all <- 'p/num_gene_snp_used_module_all.Sigma_nullz.txt'

file_z <- paste0("/project2/xuanyao/llw/eQTLGen/MSigDB_est_Sigma/z/z.module", module, ".chr", chr, ".txt.gz")
...
# remove those genes not in eqtlgen
z_mat_trans <- z_mat_trans[, colnames(z_mat_trans) %in% colnames(Sigma_null_z), drop=FALSE]
...
if(nrow(Sigma) >= 2){
  ...
  p_snp <- ModifiedPCOMerged(
    Z.mat = z_mat_trans_cross[ind_snp_shared_sigma, , drop=FALSE],
    Sigma = Sigma,
    SigmaO = SigmaO
  )
  p_all <- c(p_all, p_snp)
