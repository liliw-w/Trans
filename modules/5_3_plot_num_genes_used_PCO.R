...
for(module in module_seq){
  ...
  fig <- ggplot(fig_dat, aes(x = factor(SNPChr))) +
    ...
    geom_line(aes(y = module_size_eqtlgen, color = "Module size (eQTLGen)"), group = NA, linetype = "dashed") +
  ...
  fig +
    ...
    scale_color_manual(name = "Gene type",
                       breaks = c("Module size", "Module size (eQTLGen)", "After cis", "After cis+cross"),
                       values = c("Module size" = "black", "Module size (eQTLGen)" = "#6d6462", "After cis" = "blue", "After cis+cross" = "#990000"),
                       guide = guide_legend() ) +
    guides(color = guide_legend(nrow = 2))
  
  # save
  ggsave(paste0("plot/num_gene_snp_used_M", module, ".pdf"), width = 6, height = 5)
