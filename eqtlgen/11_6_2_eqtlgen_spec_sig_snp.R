...
file_out <- paste0('postanalysis/eqtlgen_spec_snp_to_dgn_ratio_', ratio, '.txt')
...
dgn_sig <- dgn_sig %>%
  ...
  distinct(SNP_id, chr, pos)

eqtlgen_sig_dgn <- eqtlgen_sig_dgn %>% distinct(meta, SNPChr, SNPPos)
...
cat(
  "Out of", nrow(eqtlgen_sig_dgn), "eQTLGen signal SNPs that were also analyzed by DGN, \n\n",
  
  colSums(select(eqtlgen_sig_dgn, starts_with("if_spec"))), "are near at least one DGN signal within same loci of distance: \n\n",
  
  buffer_reg, "respectively. \n\n",
  
  sum(eqtlgen_sig_dgn$min_dis_to_dgn_sig == Inf), "are not close to any DGN signals. \n\n"
)
